...
describe('AuthValidator - Validación de entrada', () => {
  ...
  describe('validateLogin', () => {
    it('✅ debería validar credenciales correctas', () => {
      const result = AuthValidator.validateLogin({ rut: '12.345.678-5', password: 'secreta123' });
      expect(result.isValid).toBe(true);
    });
    ...
    it('❌ debería rechazar RUT inválido', () => {
      const result = AuthValidator.validateLogin({ rut: '12.345.678-9', password: 'secreta123' });
      expect(result.isValid).toBe(false);
      expect(result.error).toContain('RUT');
    });
  });
