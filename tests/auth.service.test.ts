...
jest.mock('../src/config/firebase', () => {
  ...
  const auth = { verifyIdToken: jest.fn(), createCustomToken: jest.fn(), setCustomUserClaims: jest.fn() };
...
function getPerfilRepoMock(): any {
  return (jest.requireMock('../src/repositories/perfil-brigadista.repository') as any).PerfilBrigadistaRepository;
}
...
describe('AuthService - Servicio de autenticación', () => {
  ...
  describe('registerGuestUser', () => {
    const guestData = { nombre: 'Test', rut: '12.345.678-5', apellido: 'Test', telefono: '912345678' };

    it('debería crear usuario invitado exitosamente', async () => {
      const userRepo = getUserRepoMock();
      const firebase = getFirebaseMock();
      const mockGuest = { id: 1, nombre: 'Test', rol: UserRole.INVITADO, estado: UserStatus.ACTIVO, firebase_uid: 'fb-uid' };
    ...
    it('debería vincular firebase_uid si existe pero no vinculado', async () => {
      ...
      const result: any = await AuthService.registerGuestUser({ ...guestData, firebase_uid: 'new-fb-uid' });
      expect(result.isNew).toBe(false);
  ...
  describe('upgradeAccount', () => {
    ...
    it('debería rechazar upgrade si usuario no existe', async () => {
      ...
      await expect(AuthService.upgradeAccount('no-existe', 'secreta123')).rejects.toMatchObject({
        statusCode: 404,
      });
    });
  });
  ...
  describe('registerFullUser', () => {
    it('debería registrar usuario completo exitosamente', async () => {
      const userRepo = getUserRepoMock();
      const firebase = getFirebaseMock();
      const mockUser = { id: 1, nombre: 'Juan', rut: '12345678-5', rol: UserRole.USUARIO, estado: UserStatus.ACTIVO };
      ...
      const result: any = await AuthService.registerFullUser({
        nombre: 'Juan', apellido: 'Pérez', rut: '12.345.678-5', telefono: '912345678',
        password: 'secreta123', token: 'valid-firebase-token-12345',
      });
    ...
    it('debería rechazar registro con datos duplicados', async () => {
      ...
      await expect(AuthService.registerFullUser({
        nombre: 'Juan', apellido: 'Pérez', rut: '12.345.678-5', telefono: '912345678',
        password: 'secreta123', token: 'valid-firebase-token-12345',
  ...
  describe('loginWithGoogle', () => {
    it('debería autenticar con Google exitosamente (nuevo usuario)', async () => {
      ...
      firebase.verifyIdToken.mockResolvedValue({
        uid: firebaseUid, email: 'juan@gmail.com',
        name: 'Juan Pérez', picture: 'https://example.com/photo.jpg',
      });
      userRepo.findByFirebaseUid.mockResolvedValue(null);
      userRepo.findByEmail.mockResolvedValue(null);
      userRepo.createFullUser.mockResolvedValue({ id: 1, firebase_uid: firebaseUid, nombre: 'Juan', rol: UserRole.USUARIO });
      firebase.setCustomUserClaims.mockResolvedValue(undefined);

      const result: any = await AuthService.loginWithGoogle({ token: 'google-auth-token-for-testing' });
    ...
    it('debería rechazar token de Google inválido', async () => {
      ...
      await expect(AuthService.loginWithGoogle({ token: 'invalid-but-20char-token!' })).rejects.toMatchObject({
        statusCode: 401,
      });
    });
  });
  ...
  describe('getUserProfile', () => {
    ...
    it('debería lanzar 404 si el usuario no existe', async () => {
      ...
      await expect(AuthService.getUserProfile(999)).rejects.toMatchObject({ statusCode: 404 });
    });
  });
  ...
  describe('updateUserProfile', () => {
    ...
    it('debería eliminar campos sensibles', async () => {
      ...
      await AuthService.updateUserProfile(1, { rol: UserRole.ADMIN, estado: UserStatus.ACTIVO, firebase_uid: 'otro-uid', nombre: 'Juan' } as any);
    ...
    it('debería rechazar RUT duplicado', async () => {
      ...
      await expect(AuthService.updateUserProfile(1, { rut: '12.345.678-5' })).rejects.toMatchObject({ statusCode: 409 });
    });
  });
  ...
  describe('convertGuestToCitizen', () => {
    it('debería lanzar error para usuario no invitado', async () => {
      ...
      await expect(AuthService.convertGuestToCitizen('fb-uid', 'secreta123')).rejects.toMatchObject({
        statusCode: 400,
      });
    });
  ...
  describe('updatePerfilBrigadista', () => {
    ...
    it('debería actualizar perfil correctamente', async () => {
      ...
      const result: any = await AuthService.updatePerfilBrigadista(1, { organismo: 'Bomberos' });
  ...
  describe('adminCreateBrigadista', () => {
    it('debería crear brigadista exitosamente', async () => {
      ...
      userRepo.findById.mockResolvedValue({ id: 1, nombre: 'Brigadista', rol: UserRole.USUARIO });
      perfilRepo.findByUsuarioId.mockResolvedValue(null);
      perfilRepo.create.mockResolvedValue({ id: 1, usuario_id: 1 });
