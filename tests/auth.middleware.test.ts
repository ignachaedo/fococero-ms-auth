...
jest.mock('../src/repositories/user.repository', () => ({ UserRepository: { findByFirebaseUid: mockFindByFirebaseUid } }));
...
describe('validateFirebaseToken', () => {
  ...
  it('debería rechazar usuario bloqueado con 403', async () => {
    req.headers.authorization = 'Bearer valid-token';
    mockVerifyIdToken.mockResolvedValue({ uid: 'firebase-uid-123' });
    mockFindByFirebaseUid.mockResolvedValue({ id: 1, estado: UserStatus.BLOQUEADO, firebase_uid: 'firebase-uid-123' });
    await validateFirebaseToken(req, res, next);
    expect(statusMock).toHaveBeenCalledWith(403);
  });

  it('debería permitir usuario activo y autenticado', async () => {
    const mockUser = { id: 1, rut: '12345678-5', nombre: 'Juan', estado: UserStatus.ACTIVO, rol: UserRole.USUARIO, firebase_uid: 'firebase-uid-123' };
    req.headers.authorization = 'Bearer valid-token';
    mockVerifyIdToken.mockResolvedValue({ uid: 'firebase-uid-123' });
