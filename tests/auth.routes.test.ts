...
describe('Rutas de autenticación - HTTP Integration', () => {
  ...
  describe('POST /login', () => {
    it('debería autenticar usuario con credenciales válidas', async () => {
      ...
      userRepo.findByRut.mockResolvedValue({
        id: 1, rut: '12345678-5', nombre: 'Juan', password: 'hashed',
        firebase_uid: 'firebase-uid', rol: 'usuario', estado: 'activo',
      });
  ...
  describe('POST /register-guest', () => {
    it('debería registrar un nuevo invitado', async () => {
      ...
      userRepo.createGuest.mockResolvedValue({
        id: 1, rut: '12345678-5', nombre: 'Juan', rol: 'invitado',
      });

      const response = await request(app)
        .post('/register-guest')
        .send({ rut: '12.345.678-5', nombre: 'Juan', apellido: 'Pérez', telefono: '912345678' });
  ...
  describe('POST /register-full', () => {
    it('debería registrar usuario completo', async () => {
      ...
      userRepo.createFullUser.mockResolvedValue({ id: 1, rut: '12345678-5', nombre: 'Juan', rol: 'usuario' });
      firebase.setCustomUserClaims.mockResolvedValue(undefined);

      const response = await request(app)
        .post('/register-full')
        .send({ rut: '12.345.678-5', nombre: 'Juan', apellido: 'Pérez', telefono: '912345678', token: 'valid-firebase-token-12345' });
  ...
  describe('POST /google', () => {
    it('debería autenticar con Google', async () => {
      ...
      firebase.verifyIdToken.mockResolvedValue({ uid: 'google-uid-123', email: 'test@example.com', name: 'Test User' });
      userRepo.findByFirebaseUid.mockResolvedValue(null);
      userRepo.findByEmail.mockResolvedValue(null);
      userRepo.createFullUser.mockResolvedValue({ id: 1, rut: 'GGGOOGLEUI', nombre: 'Test', apellido: 'User', rol: 'usuario' });
      firebase.setCustomUserClaims.mockResolvedValue(undefined);

      const response = await request(app).post('/google').send({ token: 'valid-google-token-for-testing' });
      expect(response.status).toBe(200);
      expect(response.body.ok).toBe(true);
    });
  ...
  describe('Rutas protegidas', () => {
    ...
    it('GET /me debería retornar 403 cuando el usuario no existe en DB', async () => {
      ...
      const response = await request(app).get('/me').set('Authorization', 'Bearer some-token');
    ...
    it('GET /me debería retornar perfil con token válido', async () => {
      ...
      userRepo.findByFirebaseUid.mockResolvedValue({ id: 1, rut: '12345678-5', nombre: 'Juan', apellido: 'Pérez', estado: 'activo' });
      userRepo.findById.mockResolvedValue({ id: 1, rut: '12345678-5', nombre: 'Juan', apellido: 'Pérez', estado: 'activo' });

      const response = await request(app).get('/me').set('Authorization', 'Bearer valid-token');
      expect(response.status).toBe(200);
      expect(response.body.ok).toBe(true);
    });
  });
  ...
  describe('Rutas de administración', () => {
    ...
    it('GET /users debería retornar 403 si no es admin', async () => {
      ...
      userRepo.findByFirebaseUid.mockResolvedValue({ id: 2, rut: '87654321-5', nombre: 'User', rol: 'usuario', estado: 'activo' });
    ...
    it('GET /users debería retornar lista si es admin', async () => {
      ...
      userRepo.findByFirebaseUid.mockResolvedValue({ id: 1, rut: '11111111-1', nombre: 'Admin', rol: 'admin', estado: 'activo' });
      userRepo.findAll.mockResolvedValue([{ id: 1, nombre: 'Admin' }, { id: 2, nombre: 'User' }]);
  ...
  describe('Catch-all 404', () => {
    it('debería retornar 404 para rutas inexistentes (con token válido)', async () => {
      ...
      firebase.verifyIdToken.mockResolvedValue({ uid: 'valid-uid' });
      userRepo.findByFirebaseUid.mockResolvedValue({ id: 1, rut: '12345678-5', nombre: 'Test', estado: 'activo' });
